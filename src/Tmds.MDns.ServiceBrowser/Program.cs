    ...
    class Program
    {
        ...
        static void printService(char startChar, ServiceAnnouncement service)
        {
            lock (s_gate)
            {
                ...
                Console.WriteLine("\tHost: {0} ({1})", service.Hostname, string.Join(", ", service.Addresses.Select(a=>a.ToString()).ToArray()));
                Console.WriteLine("\tPort: {0}", service.Port);
                Console.WriteLine("\tTxt : [{0}]", string.Join(", ", service.Txt.ToArray()));
            }
        }
    }
}
